...
public class DepartmentGenerator implements GeneratorInterface {
	
	public static Department CreateDepartmentAndAssignTeachers(String departmentName, List<Teacher> tList) {
		return new Department(departmentName, tList);
	}
	...
	public static List<Department> CreateDepartmentAndAssignTeachers(int numberOfDept, List<Teacher> teacher_DB) {
		...
		for (int i = 0; i < numberOfDept; i++) {
			...
			if (numOfAssignedTeachers + teacherNumOfCurrentDept >= teacher_DB.size()) {
				...
				dList.add(CreateDepartmentAndAssignTeachers(DEPARTMENT_NAME[i], tList));
